                ...
                for ($i=0; $i<count($store_view_id); $i++) {
                    ...
                    if ($include_sku) {
                      ...
                      //  $productCollection = $category->getProductCollection()->addAttributeToSelect('*');
                        $productCollection = $this->_objectManager->create('Magento\Catalog\Model\ResourceModel\Product\Collection')->addAttributeToSelect('*')->addCategoriesFilter(['in' => $cat_data->getData('entity_id')]);
                        $d =0;
                        $productid = '';
