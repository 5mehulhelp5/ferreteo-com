...
class Data extends AbstractHelper
{
    ...
    const ORDERTRACKING_ZOOM_ENABLE = 'ordertracking/zoom/enable';
    const ORDERTRACKING_ZOOM_SEARCHTYPE = 'ordertracking/zoom/search_type';
    const ORDERTRACKING_ZOOM_CLIENTCODE = 'ordertracking/zoom/client_code';
    ...
    {
        return $this->scopeConfig->getValue(self::ORDERTRACKING_ZOOM_ENABLE, ScopeInterface::SCOPE_STORE);
    }
    ...
    public function getZoomSearchType()
    {
        return $this->scopeConfig->getValue(self::ORDERTRACKING_ZOOM_SEARCHTYPE, ScopeInterface::SCOPE_STORE);
    }

    public function getClientcode()
    {
        return $this->scopeConfig->getValue(self::ORDERTRACKING_ZOOM_CLIENTCODE, ScopeInterface::SCOPE_STORE);
    }
     public function getZoomData($waybillnumber)
    {
        $html = '';
        try {
            if ($waybillnumber != '') {
                $clientcode = $this->getClientcode();
                $searchtype = $this->getZoomSearchType();
                $curl = curl_init();
                curl_setopt_array($curl, array(
                    CURLOPT_RETURNTRANSFER => 1,
                    CURLOPT_URL => "http://sandbox.grupozoom.com/baaszoom/public/canguroazul/getLastTracking?tipo_busqueda=" . $searchtype . "&codigo=" . $waybillnumber . "&codigo_cliente=" . $clientcode,
                    CURLOPT_USERAGENT => "Codular Sample cURL Request"
                ));
                ...
                if (array_key_exists('0', $arr['entidadRespuesta'])) {
                    
                    $html .= '<div class="alldate">';
                    $html .= '<p class="status-class">' . __('STATUS : ') . $arr['entidadRespuesta'][0]['descripcion_estatus'] . '</p>';
                    ...
                    $html .= '<p>' . __('Status') . '</p>';
                    $html .= '</div>';
                    foreach ($arr['entidadRespuesta'] as $scan) {


                        $html .= '<div class="scan-row">';
                        $html .= '<p>' . $scan['fecha'] . '</p>';
                        $html .= '<p>' . $scan['siglas'] . '</p>';
                        $html .= '<p>' . $scan['descripcion_estatus'] . '</p>';
                        $html .= '</div>';
                    }
                    $html .= '</div>';
                    $html .= '</div>';
                    ...
                    $html .= '<div class="alldate">';
                    $html .= '<p class="status-class">' . __('STATUS : ') . $arr['entidadRespuesta']['descripcion_estatus'] . '</p>';
                    ...
                    $html .= '<div class="scan-row">';
                    $html .= '<p>' . $arr['entidadRespuesta']['fecha'] . '</p>';
                    $html .= '<p>' . $arr['entidadRespuesta']['siglas'] . '</p>';
                    $html .= '<p>' . $arr['entidadRespuesta']['descripcion_estatus'] . '</p>';
                    $html .= '</div>';
                    //   }
                    $html .= '</div>';
                    $html .= '</div>';
