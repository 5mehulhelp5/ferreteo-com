    ...
    public function execute()
    {
        ...
        foreach ($model as $ans) {
            ...
            if ($ans->getRespondType()=='Admin') {
                $class='wk_bold wk_admin';
            } elseif ($ans->getRespondType()=='Seller') {
                $class='wk_bold wk_seller';
            } else {
                $class='wk_bold wk_customer';
            }
            $answer=$answer.'<div class="extra-answ"><span class="wk_prewrap">'.$ans->getContent().'</span>
                <div class="user-info1">by 
                <span class="'.$class.'">'.$ans->getRespondNickname().'</span> on 
                <span>'.$this->_timezone->formatDate($ans->getCreatedAt(), \IntlDateFormatter::MEDIUM).'</span><br/>
                <div class="reviews">
                <span class="'.$like.'" title="like" dataid="'.$ans->getAnswerId().'"></span><span>'.$likes.'</span>
                <span class="'.$dislike.'" title="dislike" dataid="'.$ans->getAnswerId().'"></span>
         ...
         $answer=$answer.
         '<div class="answe">
         <a class="qa-ans"><button class="action primary"><span>'.__('Give Answer').'</span></button></a>
         </div>';
