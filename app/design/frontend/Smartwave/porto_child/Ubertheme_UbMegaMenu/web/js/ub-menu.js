    ...
    $.widget('mage.ubMenu', {
            ...
            $(self.options.rootSelector).find(self.options.itemSelector).each(function(i, el) {
                if (!$(el).hasClass('tab-head')) {
                    $(el).mouseenter(function() {
                        ...
                        //var contentTop = $(".mega-menu-content .content-top").height();
                        var calScroll = $(".mega-menu-content").scrollTop();
                        // console.log(calScroll);
                        var top = position.top - calScroll;
                        $(el).children('.child-content').first().css("top", top);
                        if ($(this).hasClass('has-child')
                        ...
                        if (!$(this).parents(self.options.itemSelector + '.has-child').length) {
                            ...
                            $(el).children('.child-content').first().css("top", "auto");
                        }
                    });
                }
            });
        ...
        drillDown: function ($next, opts) {
            ...
            self.doDrilling(
                ...
                function () {
                    ...
                    self.options.drillOptions.$container.parent().parent().parent().addClass('drilling');
                    self.options.drillOptions.$container.parent().parent().parent().scrollTop(0);
                    self.options.drillOptions.$container.parent().find('.btn-drilldown').show();

                    //fixed styles of the header links if exists
                    var $headerLinks = self.options.drillOptions.$container.parent().siblings('.header.links');
                    self.fixPosition($headerLinks);
                }
            );
        },
            ...
            self.doDrilling(
                ...
                function () {
                    ...
                    if ($back.hasClass('level0')) {
                        self.options.drillOptions.$container.parent().parent().parent().removeClass('drilling');
        ...
        fixPosition: function ($el) {
            if ($el.length) {
                ...
                $el.css('top', $drilldownRoot.outerHeight());
